...
| `StructuralGoldbach/SelfContainment.lean` | The necessity side of additive self-containment: `additiveSelfContained_of_hasGoldbachRep` — if a representation of `n` exists and `n ≤ B+2`, it is forced into base `≤ B` on both sides, unconditionally, zero `native_decide`. Complements (does not replace) `windowCovered`'s existence claim; `B+2` shown sharp by explicit example |
